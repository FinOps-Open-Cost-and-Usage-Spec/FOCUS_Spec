...
* The user-defined selection of columns includes the columns documented for a selected detail variant, so those columns are delivered without an exception to the column-selection requirement.
...
* **Column selection and detail variant columns.** The column-selection requirement, introduced in 1.4, says a FOCUS dataset is configurable to include only a user-defined selection of columns, while selecting a detail variant brings in the columns documented for that variant. This PR treats those columns as part of the user-defined selection: a column list and a detail variant choice are both user configuration, so the delivered column set is the union of the two and "only" keeps its force. The 1.4 requirement is therefore unchanged, and `ATT-DatasetConfiguration-A-001-M` stays accurate. Alternatives: add `except when a column is documented for a selected detail variant` to the column-selection requirement, which states the carve-out explicitly but amends text introduced in 1.4; or leave both requirements unqualified and rely on the detail variant requirement taking precedence implicitly, which a validator reading A-001 alone would treat as a conflict. Which resolution does TF-2 prefer?
