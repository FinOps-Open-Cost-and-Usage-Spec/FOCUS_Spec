...
Neither the "user" nor the "feature" detail variant is more detailed than the other, since each populates a column the other does not. The records in the detail scope sum to the same totals for each summable metric under any of the three selections.
