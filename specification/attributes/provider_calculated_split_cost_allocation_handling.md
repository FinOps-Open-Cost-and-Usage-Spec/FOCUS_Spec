...
* A Cost and Usage *FOCUS dataset* MUST include the following columns when the provider supports provider-calculated split cost allocation:
...
* A Cost and Usage *FOCUS dataset* SHOULD include the following column when the provider supports provider-calculated split cost allocation:
...
* For each allocated charge record in a Cost and Usage *FOCUS dataset*, all dimension columns and non-aggregatable metric columns MUST match the values of the origin charge record.
