...
[**CSV Example**](/specification/data/ai_billing/drawdown_via_prepayment_foundation_model_api.csv)
...
* Usage rows use [PricingCategory](#datamodel.costandusage.pricingcategory) "Committed" with contracted unit prices 20% below list ($4.00 / $20.00 per 1,000,000 tokens vs $5.00 / $25.00), so [ListCost](#datamodel.costandusage.listcost) exceeds [EffectiveCost](#datamodel.costandusage.effectivecost).
* On the usage rows, [BilledCost](#datamodel.costandusage.billedcost) is $0.00 while [EffectiveCost](#datamodel.costandusage.effectivecost) carries the amortized drawdown ($20.00 input and $30.00 output), matching the covering-purchase pattern used elsewhere for commitment discounts.
* Solora AI is both the [*service provider*](#glossary:service-provider) and the [*invoice issuer*](#glossary:invoice-issuer). Each token type remains a separate [*SKU*](#glossary:sku), with the TokenType property of [SkuPriceDetails](#datamodel.costandusage.skupricedetails) labeling the usage rows "Input" or "Output".
