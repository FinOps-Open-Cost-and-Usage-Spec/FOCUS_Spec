...
| [Examples: AI Prompt Caching](#appendix.examples:aipromptcaching) | Illustrates how to distinguish cached from uncached [*token*](#glossary:token) charges in a Cost and Usage FOCUS dataset, covering a provider that prices cache retention into the write and one that charges for cache storage separately. |
