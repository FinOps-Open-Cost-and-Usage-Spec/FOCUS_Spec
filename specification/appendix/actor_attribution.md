...
7. **Seat-Plus-Token SaaS:** **Acme Corp** subscribes to PipelCRM and uses its embedded AI assistant. Neither the seat license nor the per-token AI usage is tied to an originating requestor (`PrincipalId` is `null`); both are reported as separate line items under the same opaque identifier (`ConsumerId`), so chargeback follows the user as service providers shift from seat-based to hybrid seat and consumption-based pricing.
