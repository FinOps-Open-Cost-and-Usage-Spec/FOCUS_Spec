...
7. **Credit:** **Acme Corp** receives a promotional credit from LatticeScale. Since the charge was generated by the provider's billing system rather than initiated by a request for a resource or service, the principal (`PrincipalId`) cannot be resolved. As the credit is applied at the account level, the charge cannot be associated with a specific end-user (`ConsumerId`).
8. **Tax:** **Acme Corp** is charged applicable tax on LatticeScale services. The tax is levied by the relevant authorities and computed by LatticeScale's billing system. Since the charge was generated by the provider's billing system rather than initiated by a request for a resource or service, the principal (`PrincipalId`) cannot be resolved. As the tax is applied at the account level, the charge cannot be associated with a specific end-user (`ConsumerId`).
...
| 7 | **Credit** | LatticeScale | `null` | `null` |
| 8 | **Tax** | LatticeScale | `null` | `null` |
