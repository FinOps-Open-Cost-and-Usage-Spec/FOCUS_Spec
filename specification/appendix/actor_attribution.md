...
6. **Seat-Plus-Token SaaS:** **Acme Corp** subscribes to PipelCRM and uses its embedded AI assistant. The seat license and per-token AI usage post as separate line items under the same opaque `ConsumerId`, so chargeback follows the user as vendors shift from seat-only to seat + consumption pricing.
