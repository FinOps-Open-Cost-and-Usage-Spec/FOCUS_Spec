...
5. **Seat-Plus-Token SaaS:** **Acme Corp** subscribes to PipelCRM and uses its embedded AI assistant. The seat license and the per-token AI usage are reported as separate line items under the same seat holder (`PrincipalId`), so attribution follows the user as *service providers* shift from seat-based to hybrid seat and consumption-based pricing.
