...
* `rec-beacon-000101` is a Security proposal with a Recommendation Status of "Deferred", representing a finding that has been reviewed and accepted as valid but scheduled for later action. Recommendation Details uses custom "x_" prefixed keys, because the supporting detail for a public access finding has no FOCUS-defined equivalent.
