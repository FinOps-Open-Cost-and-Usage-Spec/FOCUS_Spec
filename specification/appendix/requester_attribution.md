...
* Principal ID is the attribution dimension. A *principal* persists while the *credentials* that authenticate it rotate, so grouping by Principal ID combines a user's or workload's *charges* regardless of how each request was credentialed.
...
Either identifier is null when the *charge* is not associated with an entity of that kind. Credential ID is also null when the *service provider* publishes no identifier for the *credential* presented.

The two columns are independent: a *charge* may carry a *principal* with no *credential* identifier (Scenarios 7 through 11), a *credential* with no *principal* (Scenario 13), both (Scenarios 1 through 6, and 12), or neither (Scenario 14). Neither column repeats the other's value, and column placement follows the *service provider's* identity and access management model rather than the kind of artifact presented: an API key is the *credential* where it authenticates a *principal* behind it (Scenario 1), and is itself the *principal* where access is granted to the key directly (Scenario 11).
...
The Principal Type and Credential Type columns are not FOCUS columns. They show the value a data generator would carry in the `Type` property of the `Principal` and `Credential` entries in [Requester Details](#datamodel.costandusage.requesterdetails), and are included here to contrast the values that could be used in different scenarios.
