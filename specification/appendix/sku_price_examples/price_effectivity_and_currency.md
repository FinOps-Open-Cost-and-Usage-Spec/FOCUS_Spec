...
* The third record carries a [ChargeCategory](#datamodel.skuprice.chargecategory) of "Credit" and describes the same credit granted rather than sold. Its ListUnitPrice of 0.010000 is the unit value the service provider publishes for the grant, which is what makes a granted balance reportable against a financial figure. Both PurchaseDurationType and PurchasePaymentModel are null, because a grant is not a purchase. It carries its own [SkuPriceId](#datamodel.skuprice.skupriceid) of "AURAWEB-GLOBAL-CREDITS-PROMOTIONAL" while sharing the [SkuId](#datamodel.skuprice.skuid) of the purchased balance, so the two price points for one credit coexist without colliding on row uniqueness.
