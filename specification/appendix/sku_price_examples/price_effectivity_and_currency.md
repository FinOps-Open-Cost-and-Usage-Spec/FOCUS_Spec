...
* The legacy storage tier carries a null SkuPriceEffectiveStart on its earlier record. That rate applied before the [*price list's*](#glossary:price-list) history begins and Aura Web publishes no origin date for it, so the record has no lower boundary and any charge before 2026-04-01 resolves to it.
...
* Both records share a SkuPriceId and differ only in PricingCurrency and [UnitPrice](#datamodel.skuprice.unitprice). PricingCurrency is a member of the composite key, so the two prices coexist as separate records for one price point.
