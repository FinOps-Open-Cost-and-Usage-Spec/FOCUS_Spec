...
Aura Web prices object storage in three quantity tiers, and grants Acme Corp a negotiated unit price on the middle tier. Tiered pricing is represented as one record per tier, each carrying its own SKU Price ID, unit price, and quantity boundaries.

[**CSV Example**](/specification/data/sku_price_examples/sku_price_quantity_tiers.csv)

Note the following details in the example dataset:

* Each tier carries its own [SkuPriceId](#datamodel.skuprice.skupriceid) that encodes its quantity boundaries — "AURAWEB-USEAST1-STORAGE-OBJECT-STANDARD-TIER-0-51200", "AURAWEB-USEAST1-STORAGE-OBJECT-STANDARD-TIER-51200-512000", and "AURAWEB-USEAST1-STORAGE-OBJECT-STANDARD-TIER-OVER-512000". Distinct SkuPriceId values are what allow the tiers to coexist as separate records for the same SKU.
...
| Quantity range | QuantityTierMinimum | QuantityTierMaximum | [ListUnitPrice](#datamodel.skuprice.listunitprice) |
| :--- | ---: | ---: | ---: |
...
* The fourth record is a negotiated unit price on the middle tier, carrying a populated [ContractId](#datamodel.skuprice.contractid) and a [ContractedUnitPrice](#datamodel.skuprice.contractedunitprice) of 0.018700 against a ListUnitPrice of 0.022000. It repeats the tier boundaries of the public middle tier rather than defining new ones, because the negotiation changed the price within that tier rather than where the tier begins and ends.
* Only one member of the composite key differs between the negotiated record and the public middle tier: ContractId. Both records share the same SkuPriceId encoding the middle tier boundaries ("AURAWEB-USEAST1-STORAGE-OBJECT-STANDARD-TIER-51200-512000") and describe the same quantity interval without colliding.
