...
* As in Scenario A, the input and output split is structural and labeled by TokenDirection, and the model-identity properties are common to both rows. Neither row carries TokenCacheAction: LatticeScale bills tokens placed into a cache on its ordinary input meter, so that *SKU* does not distinguish them from input tokens that were not cached.
