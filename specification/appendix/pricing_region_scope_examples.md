...
* [RegionId](#datamodel.costandusage.regionid) and PricingRegionScope answer different questions. All four rows carry a RegionId of "us-east-1", the region Aura Web anchors the offering to, and only PricingRegionScope shows that two of them were priced at the global rate rather than the regional one.
* Neither a populated nor a null RegionId identifies global pricing on its own. Here the globally priced rows carry the same RegionId as the region-pinned rows; in Scenario B the global row carries none, and RegionId is also null for [*services*](#glossary:service) that are simply not regionalized. PricingRegionScope is what separates these cases.
...
* The data zone row carries a RegionId of "northeurope" and the regional row carries "westeurope". Both regions sit inside the European area CrestNode prices as one data zone, and RegionId names only the region a row ran in, so it does not show that the first was priced across that whole area and the second for its own region alone. PricingRegionScope is what carries that. The global row carries no RegionId, because CrestNode does not anchor a globally priced offering to a region. Not every *service provider* leaves it null, as Aura Web does not in Scenario A, so a null RegionId is one shape among several rather than a marker of global pricing.
