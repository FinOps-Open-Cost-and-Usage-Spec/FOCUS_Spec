...
* Provider-defined tags {+MUST+} adhere to the following[-additional-] requirements:
  * Provider-defined tag keys MUST be prefixed with a predetermined, provider-specified tag key prefix that is unique to each corresponding provider-specified [-tag scheme.-]{+[*tag scheme*](#glossary:tag-scheme).+}
