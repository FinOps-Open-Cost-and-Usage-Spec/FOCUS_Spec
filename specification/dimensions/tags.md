...
A tag becomes *finalized* when a single value is selected from a set of possible tag values assigned to the tag key.  This can occur when a tag is set at different levels of a resource grouping hierarchy (if supported by the Provider), or a tag is value is determined from a set of user-defined rules.
