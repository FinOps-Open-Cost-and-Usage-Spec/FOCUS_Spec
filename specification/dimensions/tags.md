...
Because the the Virtual Machine Resource does not have an `env` tag, `env:prod` (italicized) was inherited from its parent Sub Account.  Conversely, because the Virtual Machine Resource already has a `team` tag (`team:web`), it does not inherit `team:ops` from its parent Sub Account.
