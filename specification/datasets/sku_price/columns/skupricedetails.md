...
SKU Price Details represent a list of [*SKU Price*](#glossary:sku-price) properties (key-value pairs) associated with a specific [SKU Price ID](#datasets.skuprice.skupriceid). These properties include qualitative and quantitative properties of a [*SKU*](#glossary:sku) (e.g., functionality and technical specifications), along with core stable pricing properties (e.g., pricing [*periods*](#glossary:period), tiers), excluding dynamic or negotiable pricing elements such as the actual unit price amounts or currency.

The composition of properties associated with a specific *SKU Price* may differ across service providers and across *SKUs* within the same service provider. SKU Price Details helps practitioners understand and distinguish *SKU Prices* within a catalog, each identified by a SKU Price ID. It provides the necessary context to determine the quantity of units for a property when it holds a numeric value (e.g., CoreCount), supporting capabilities such as unit economics and normalized price comparisons.
...
* When SkuPriceDetails is not null, SkuPriceDetails MUST adhere to the following requirements:
  ...
  * Property value MUST represent the value for a single [PricingUnit](#datasets.skuprice.pricingunit) when the property holds a numeric value.
...
<br><sup>2</sup> Memory manufacturers still commonly use "GB" to refer to 2<sup>30</sup> bytes, which is known as GiB in other contexts.
