...
* When included, TokenCacheAction and TokenDirection MUST adhere to the following requirements:
  ...
  * TokenCacheAction MUST be "Uncached" when the *SKU Price* meters tokens without distinguishing a cache interaction (e.g., tokens generated in a response, or request tokens that a service provider bills on one meter whether or not they were placed into a cache).
