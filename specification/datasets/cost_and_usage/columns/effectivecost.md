...
Effective Cost is denominated in the [Billing Currency](#datamodel.costandusage.billingcurrency). Effective Cost is commonly used to support FinOps activities, including [*accrual-based*](#glossary:accrual-based-accounting) reporting, forecasting, and cost allocation.
...
* EffectiveCost MUST equal BilledCost when [ChargeCategory](#datamodel.costandusage.chargecategory) is "Usage" and the *charge* is not *covered* by other eligible *charges*.
...
* EffectiveCost MUST be 0 when ChargeCategory is "Purchase" and the purchase is intended to cover related eligible *charges*. This requirement applies even when the *covered charges* originate from different [CostAndUsage](#datamodel.costandusage) [*dataset instances*](#glossary:dataset-instance), possibly from a different [ServiceProviderName](#datamodel.costandusage.serviceprovidername).
...
* The sum of EffectiveCost across all related *covering* and *covered charges* MUST equal the sum of BilledCost across the same set of *charges*, within the [*charge period*](#glossary:chargeperiod) of the *covering charges*, when both the *covering* and *covered charges* are present in the *dataset instance*.
* The sum of EffectiveCost for a given [*billing period*](#glossary:billing-period) MAY differ from the sum of BilledCost when *covered* and *covering charges* span multiple *billing periods* or [*billing accounts*](#glossary:billing-account), or when only one side of a covering relationship is present in the *dataset instance*.
