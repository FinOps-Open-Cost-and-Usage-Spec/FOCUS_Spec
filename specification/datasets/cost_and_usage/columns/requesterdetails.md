...
* RequesterDetails MUST be of type JSON Array (serialized as a String where necessary).
...
Every entry value uses the same set of properties, so the *principal*, the *credential*, and any additional level share one shape. Each further level at which a *requester* is represented (e.g., a delegating identity, a client application) is carried as a custom entry whose value uses those same properties. A client application (e.g., a command-line agent, an IDE extension, a hosted agent runtime) is a level of this kind when the *service provider* defines it in its identity and access management model and identifies which one made each request, so *charges* from one *principal* through several client applications carry a different entry for each. Where the *service provider* does not define client applications but records which one a *credential* was issued to, that record is an attribute of the *credential* and is carried as a custom property within the `Credential` entry.
...
A set of properties describing the *requester* associated with a *charge*.
