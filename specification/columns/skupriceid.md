...
* When SkuPriceId is not null, SkuPriceId adheres to the following additional requirements:
  ...
  * SkuPriceId MUST serve as a reference for a *SKU price* in a provider-published *price list*, allowing lookup of detailed information about the *SKU Price*.
  * SkuPriceId MUST enable lookup of the given [ListUnitPrice](#listunitprice) in a provider-published *price list* when the provider publishes unit prices exclusive of discounts.
  * SkuPriceId MUST enable lookup of the given [ContractedUnitPrice](#contractedunitprice) in a provider-published *price list* when the provider supports negotiated pricing concepts.
