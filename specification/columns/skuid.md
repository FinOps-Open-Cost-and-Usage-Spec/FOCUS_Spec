...
The SkuId column MUST be present in the billing data when the provider publishes a SKU list. This column MUST be of type String. It MUST NOT be null when [ChargeClass](#chargeclass) is "Regular" and [ChargeCategory](#chargecategory) is "Usage" or "Purchase", and MUST be null when ChargeCategory is "Tax". For all other combinations of ChargeClass and ChargeCategory, this column MAY be null. SkuId MUST equal SkuPriceId when a provider does not support an overarching SKU ID construct.
