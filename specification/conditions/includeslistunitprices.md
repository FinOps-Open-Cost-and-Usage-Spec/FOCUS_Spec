...
The Includes List Unit Prices condition represents a verifiable state where the operating model includes standard, non-discounted baseline unit prices, regardless of whether that price list is public or private.
...
* IncludesListUnitPrices MUST evaluate to true when the operating model provides predefined "retail" or standard unit rates (e.g., a standard price catalog or baseline rate card), even if those prices are restricted to authenticated users or private customer portals.
