...
The [charge](#glossary:charge) that was created as the result of an allocation operation. This is used in the context of [Service-Provider-Calculated Split Cost Allocation](#providercalculatedsplitcosthandling) to identify the charges that were created from the [origin charge](#glossary:origin-charge) resulting from the application of Service-Provider-Calculated Split Cost Allocation.

<a name="glossary:allocated-method"><b>Allocated Method</b></a>

The process or formula by which cost is being allocated from an [origin charge](#glossary:origin-charge) to produce [allocated charges](#glossary:allocated-charge). This is used in the context of [Service-Provider-Calculated Split Cost Allocation](#serviceprovidercalculatedsplitcosthandling) which requires documentation of the method to be provided for any and all allocated methods used. May also be colloquially referred to as allocation method.
...
The suggested service-provider-published unit price for a single [Pricing Unit](#pricingunit) of the associated [SKU](#glossary:sku), exclusive of any discounts. This price is denominated in the [Billing Currency](#glossary:billing-currency).
...
The [charge](#glossary:charge) that existed prior to an operation. This is used in the context of Service-Provider-Calculated Split Cost Allocation to identify the charge that existed prior to the application of [Service-Provider-Calculated Split Cost Allocation](#serviceprovidercalculatedsplitcosthandling) to produce [allocated charges](#glossary:allocated-charge).
