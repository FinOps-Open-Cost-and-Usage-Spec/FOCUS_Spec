...
* **Intrinsic gate and operating model gate.** The two gates a Conditional column records. An intrinsic gate is value variation that depends on the row (SkuId is null on tax rows), written as a row-level rule. An operating model gate is presence variation that depends on what the model includes (regions, virtual currency), written as an operating model Condition. Category variation is always an operating model gate.
