...
- **Finalized Invoice and Closed Billing Period**
  ...
  - To address these gaps in this scenario, and solely for illustrative purposes, we assume the following:
...
##### Scenario - 1: Post Invoice Correction: Correction to ResourceId dimension
...
- This record was part of the **finalized invoice** issued at T0, and the **May billing period is already closed**.
...
- For corresponding **correction records**, the following applies:
  ...
  - **Provisioning perspective**:  
    `x_ExportDateTime = T2` (i.e., the date it became available to the consumer)

  - **Operational perspective**:  
    All Correction records (increments and decrements in case of the Ledger style, negations and corrected records in case of Accounting style) belong to **May**, so `ChargePeriodStart` and `ChargePeriodEnd` **must reflect the actual usage period** (e.g., `ChargePeriodStart = 2025-05-01`)
  ...
  - **Financial perspective**:  
    ...
    - **BillingPeriod constraint**:  
      Since the billing period is closed, `BillingPeriodStart`/`BillingPeriodEnd` **must be equal to or later than** the first **open** billing period — i.e., **June 2025 or later**.
...
- This cost was not included in the finalized invoice issued at T0, and the **May billing period is already closed**.
...
- For such **late-arriving charge records**, the following applies:
  ...
  - **Operational perspective**:  
    The cost belongs to **May**, so `ChargePeriodStart` and `ChargePeriodEnd` **must reflect the actual usage period** (e.g., `ChargePeriodStart = 2025-05-01`)
