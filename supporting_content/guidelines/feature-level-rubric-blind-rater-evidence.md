...
**Edit after this round.** The variance sub-bullet was completed on the branch, so the current text is not byte-identical to what this round tested. The admissible branch now names its outcome, so the gate is admissible "making the column `Conditional`" in the same sentence that establishes admissibility. That is the one gap this round showed can move an outcome, and the added clause states a result the sub-bullet's own worked example already assumed rather than introducing a new rule. The guideline was restructured after this round, so the Revision Boundary above governs how the rule reads now. Confirming that the completed sentence removes the split is work for a later round, and PricingCurrency is the column to watch.
