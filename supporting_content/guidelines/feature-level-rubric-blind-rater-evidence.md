...
**Round 6 (2026-07-08, summary-section A/B check, three raters on distinct models per arm).** A plain-language summary section, The Rubric in Brief, was drafted for insertion after the Overview, composed only of sentences reused verbatim from the guideline and pointer lines that defer to the governing sections. Two arms ran the seven sentinels: treatment (guideline with the summary) and control (without). The canary held in both arms. ChargeClass, EffectiveCost, and AvailabilityZone were unanimous and matched expected outcomes in both arms; ListCost was unanimous in treatment and 2-1 in control; InvoiceId was 2-1 in both arms, the same rater choosing Mandatory while acknowledging the boundary. No treatment rater quoted the summary as governing material, and every treatment-arm deviation reproduced in the control, so the summary introduced no new divergence. Two base-text observations were recorded: BillingAccountName reached Mandatory for zero of six raters across both arms, through three distinct routes (the necessity refinement carve-out, principle 4's Conditional default, and applicability variance), sharpening the open design question from round 5; and in three rater-column instances a rater acknowledged the boundary yet chose Mandatory over the interim rule's Conditional default, suggesting the default's bindingness may need reinforcement. These are rater outputs under redaction, not proposed levels.
