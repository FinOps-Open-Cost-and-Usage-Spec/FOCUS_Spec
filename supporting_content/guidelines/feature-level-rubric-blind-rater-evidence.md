...
Principle numbers below are the ones in force at the time of each round. Later passes merged the two fabrication principles and added a Supported Features write-back, so the current guideline still has seven, but not the same seven: what these rounds call principle 6 is now part of principle 5, derivation moved from 7 to 6, and principle 7 is new and has not been tested.
