...
* **Derivation without a leveling rule.** Columns linked by derivation were leveled with nothing connecting them, so a derived column could be mandated redundantly, or a source column dragged to the level of something computed from it. The rubric's answer is the directional derivation principle: a derived column can be no more present than its sources, a derived column's absence never pulls a source down, and each source is leveled by its own inputs.
...
The principles guideline carries what is stable: the operating-model basis, the earned-Mandatory bar, the two axes, the directional derivation principle, the four decision inputs, the gate taxonomy, the two-layer output, and the interim boundary rule. The companion mechanics guideline carries the machinery described here: the matrix, the calibrated threshold, the records, the back-test, and a machine-readable check. The split is deliberate: the principles can be adopted and applied with the interim rule now, and nothing in them changes when calibration replaces the interim rule with a measured threshold.
