# Column: LookbackPeriodStart

## Status

Proposed as `LookbackPeriodStart` and `LookbackPeriodEnd` (Option 1 below) during review of the Recommendation dataset ([PR #2444](https://github.com/FinOps-Open-Cost-and-Usage-Spec/FOCUS_Spec/pull/2444)).

The representation question remains open. Consensus in review was to look at duration expression holistically across the dataset rather than settle it column by column, so the alternative single-column ISO 8601 form is recorded below along with the tradeoffs. Adopting Option 2 later would replace both columns.

## Problem

Recommendation engines derive a recommendation from observed behavior over some preceding window of time. Two recommendations with identical estimated cost impact are not equally trustworthy when one is based on 7 days of observation and the other on 90 days. Without the observation window in the dataset, a practitioner cannot judge the confidence of a recommendation, and cannot compare recommendations across data generators that use different default windows.

There is currently no column in the Recommendation dataset expressing the window a recommendation was derived from.

## Options discussed

### Option 1: Explicit start and end timestamps (proposed)

Two Date/Time columns, `LookbackPeriodStart` and `LookbackPeriodEnd`, both Optional and nullable, following the *inclusive start bound* / *exclusive end bound* convention used by `ChargePeriodStart` / `ChargePeriodEnd`. Both are bounded against `RecommendationCreateDate`, since a recommendation cannot be derived from behavior observed after it was generated.

Optional was chosen on two tests. A lookback period does not apply to all recommendations: the dataset spans cost, performance, reliability, and security, and a recommendation derived from configuration state (e.g., an unencrypted disk or a resource without redundancy) has no observation window at all. The dataset is also useful without these columns: prioritizing by cost impact, filtering by category, and joining to Cost and Usage all work when the columns are absent. Lookback period sharpens confidence in a recommendation rather than carrying the dataset's core value.

* Consistent with existing period columns in the specification, such as `ChargePeriodStart` / `ChargePeriodEnd` and `BillingPeriodStart` / `BillingPeriodEnd`, which are the established pattern for expressing a bounded window.
* Directly filterable and comparable without computation.
* Unambiguous about which window was actually observed, including when a generator's window is irregular or was truncated by available history.
* Costs two columns instead of one.

### Option 2: ISO 8601 duration as an offset

A single column carrying an ISO 8601 duration (e.g., `P7D`, `P30D`), interpreted as an offset back from `RecommendationCreateDate`.

* One column instead of two.
* Matches how generators typically describe their own windows ("30-day lookback").
* Requires computation to resolve to actual timestamps, and depends on `RecommendationCreateDate` as the anchor.
* Cannot express a window that does not end at the recommendation's creation time, such as a generator that observes through the end of the prior billing period.
* Introduces an ISO 8601 duration format to the dataset, which is a broader convention decision than this single column.

## Open questions

* Do generators report their intended window (e.g., a configured 30 days) or the window actually observed (e.g., 22 days of available history)? The proposed columns express the window observed, but no requirement currently compels that reading.
* Should these columns be Conditional rather than Optional? Conditional would make the columns dependably present for the recommendation categories that do derive from an observation window, which Optional does not, but it requires stating a condition that cleanly separates observation-based recommendations from configuration-state ones. That boundary is fuzzier than the contract-commitment one, so Optional was chosen pending a workable condition.
* Under Optional, a consumer must handle both a missing column and a null value as separate cases. Is that acceptable, or is it an argument for Conditional?
* If ISO 8601 durations are adopted for `ContractCommitmentDurationType`, should this column follow, replacing both columns with the Option 2 form? This is the holistic question the group deferred.
* Should a recommendation derived from a window be required to populate these columns, rather than merely permitted to?

## Related

* `ContractCommitmentDurationType` — open question on expressing duration as a quantity and unit versus a single string, discussed in [PR #2444](https://github.com/FinOps-Open-Cost-and-Usage-Spec/FOCUS_Spec/pull/2444). Both this column and that one concern how the dataset expresses durations, which is why the group preferred to resolve them together.
